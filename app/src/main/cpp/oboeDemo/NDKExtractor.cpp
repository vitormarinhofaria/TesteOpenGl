...
int32_t NDKExtractor::decode(AAsset *asset, uint8_t *targetData, AudioProperties targetProperties) {
    ...
    if (amresult != AMEDIA_OK){
        LOG("Error setting extractor data source, err %d", amresult);
        return 0;
    }
    ...
    if (AMediaFormat_getInt32(format, AMEDIAFORMAT_KEY_SAMPLE_RATE, &sampleRate)){
        LOG("Source sample rate %d", sampleRate);
        if (sampleRate != targetProperties.sampleRate){
            LOG("Input (%d) and output (%d) sample rates do not match. "
        ...
        LOG("Failed to get sample rate");
        return 0;
    };

    int32_t channelCount;
    if (AMediaFormat_getInt32(format, AMEDIAFORMAT_KEY_CHANNEL_COUNT, &channelCount)){
        LOG("Got channel count %d", channelCount);
        if (channelCount != targetProperties.channelCount){
            LOG("NDK decoder does not support different "
                 "input (%d) and output (%d) channel counts",
                 channelCount,
                 targetProperties.channelCount);
        }
    } else {
        LOG("Failed to get channel count");
        return 0;
    }

    const char *formatStr = AMediaFormat_toString(format);
    LOG("Output format %s", formatStr);

    const char *mimeType;
    if (AMediaFormat_getString(format, AMEDIAFORMAT_KEY_MIME, &mimeType)) {
        LOG("Got mime type %s", mimeType);
    } else {
        LOG("Failed to get mime type");
        return 0;
    }
        ...
        if (isExtracting){
            ...
            if (inputIndex < 0){
                if (inputIndex == AMEDIACODEC_INFO_TRY_AGAIN_LATER){
                    ...
                    LOG("Codec.dequeueInputBuffer unknown error status");
                }
            } else {
                ...
                if (sampleSize > 0){
                    ...
                    LOG("End of extractor data stream");
                    isExtracting = false;
        ...
        if (isDecoding){
            ...
            if (outputIndex >= 0){
                ...
                if (info.flags & AMEDIACODEC_BUFFER_FLAG_END_OF_STREAM){
                    LOG("Reached end of decoding stream");
                    isDecoding = false;
                } 
                ...
                switch(outputIndex){
                    ...
                    case AMEDIACODEC_INFO_OUTPUT_BUFFERS_CHANGED:
                        LOG("dequeueOutputBuffer: output buffers changed");
                        break;
                    case AMEDIACODEC_INFO_OUTPUT_FORMAT_CHANGED:
                        LOG("dequeueOutputBuffer: output outputFormat changed");
                        format = AMediaCodec_getOutputFormat(codec);
                        LOG("outputFormat changed to: %s", AMediaFormat_toString(format));
                        break;
                }
            }
        }
